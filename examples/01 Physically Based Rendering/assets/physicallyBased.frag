...
float shadowingMasking(float NdotV, float NdotL, float roughness)
{
  ...
  float shadowing = -1.0 + sqrt(r * r * l * l + 1.0) / 2.0;
  float masking = -1.0 + sqrt(r * r * v * v + 1.0) / 2.0;
  
  return (NdotL * NdotV) / (1.0 + shadowing + masking);
}

void main()
{
  vec4 albedo = vec4(1.0, 1.0, 1.0, 1.0) / PI;
  ...
  specular /= NUM_SAMPLES;
	...
	color = albedo + radiance;// * vec4(F, 1.0);
}